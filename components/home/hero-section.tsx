...
export function HeroSection() {
  ...
  // Load video after component mounts with delay for better initial loading
  useEffect(() => {
    ...
    }, 2000) // Increase delay to prioritize other content

    return () => clearTimeout(timer)
  }, [])

  return (
    <section className="relative w-full h-[85vh] max-h-[800px] min-h-[550px] overflow-hidden">
      ...
      <div className="absolute inset-0 bg-black">
        <Image
          ...
          style={{ opacity: videoLoaded ? 0 : 1, transition: "opacity 0.8s ease-in-out" }}
        />
        ...
        <video
          ...
          style={{ opacity: videoLoaded ? 1 : 0, transition: "opacity 0.8s ease-in-out" }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/50 to-black/80" />
      </div>

      {/* Content */}
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center text-white px-4 py-16 sm:py-0">
        <div className="fade-in-up bg-primary/80 backdrop-blur-sm px-4 py-1 rounded-full text-sm font-medium mb-4">
          La mejor experiencia electrónica
        </div>

        <h1 className="fade-in-up delay-100 text-4xl sm:text-5xl md:text-7xl font-bold text-center mb-4 sm:mb-6 leading-tight">
          Vive la experiencia
          <span className="text-primary block">electrónica</span>
        </h1>

        <p className="fade-in-up delay-200 text-lg sm:text-xl md:text-2xl text-center mb-6 sm:mb-10 max-w-2xl text-white/90 px-2">
        ...
        {/* Feature cards - using minimal CSS animations */}
        <div className="fade-in-up delay-400 grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4 mt-8 sm:mt-16 w-full max-w-4xl px-2">
      ...
      <div className="fade-in-up delay-500 absolute bottom-8 left-1/2 transform -translate-x-1/2 hidden sm:block">
        <div className="w-6 h-10 rounded-full border-2 border-white/30 flex justify-center pt-2">
          <div className="w-1.5 h-1.5 rounded-full bg-white simple-bounce"></div>
        </div>
      </div>
    </section>
  )
